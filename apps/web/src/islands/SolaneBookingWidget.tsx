...
export default function SolaneBookingWidget({ restaurant, initialBookings, initialEvents, initialPrivateHires, locale = 'es' }: SolaneBookingWidgetProps) {
  ...
  return (
    <form className="vw-widget sw-widget" data-solane-booking-widget onSubmit={submit} noValidate>
      ...
      <dialog className="sw-gateway" ref={gateway} aria-labelledby="solane-gateway-title" data-deposit-gateway><div><p>{localized(copy.demo, locale)}</p><h3 id="solane-gateway-title">{localized(copy.deposit.gatewayTitle, locale)}</h3><p>{localized(copy.deposit.gatewayBody, locale)}</p><dl><div><dt>{localized(copy.deposit.gatewayMenu, locale)}</dt><dd>{selectedMenu?.name ?? '—'}</dd></div><div><dt>{localized(copy.deposit.gatewayPolicy, locale)}</dt><dd>{pendingBooking?.deposit ? `${pendingBooking.deposit.breakdown.percentageBps / 100}% · ${pendingBooking.partySize}` : '—'}</dd></div><div><dt>{localized(copy.deposit.gatewayAmount, locale)}</dt><dd>{pendingBooking?.deposit ? money(pendingBooking.deposit.breakdown.amountCents) : '—'}</dd></div></dl>{pendingBooking?.deposit && <p className="sw-gateway__terms"><b>{localized(copy.deposit.gatewayTerms, locale)}</b><span>{localized(copy.deposit.terms, locale)}</span><time dateTime={pendingBooking.deposit.termsAcceptedAt}>{new Intl.DateTimeFormat(locale, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(pendingBooking.deposit.termsAcceptedAt))}</time></p>}<div><button className="vw-button vw-button--ghost" type="button" onClick={() => { gateway.current?.close(); setPendingBooking(null); }}>{localized(copy.deposit.gatewayCancel, locale)}</button><button className="vw-button" type="button" data-confirm-deposit onClick={confirmGateway}>{localized(copy.deposit.gatewayConfirm, locale)}</button></div></div></dialog>
    </form>
  );
}
