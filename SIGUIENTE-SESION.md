...
**Fase actual:** heroes v2 + F13 + F14 publicados en GitHub, preview y producción ✅ · secrets rotados y cifrados ✅ · F12 pendiente de revalidación real (2026-08-18)
**Siguiente hito:** comprobar mediante entregas autorizadas que las nuevas claves de preview y producción funcionan y conservan la idempotencia — ver `docs/ROADMAP.md` §F12

## Siguiente paso concreto

1. Obtener autorización explícita para generar un correo de prueba en preview y otro en producción; no reutilizar referencias antiguas ni datos personales reales.
2. En cada entorno, enviar un payload rotulado `PRUEBA INTERNA - NO CONTACTAR`, repetir exactamente el mismo JSON y comprobar `202 delivered` seguido de `replayed: true` con la misma referencia.
3. Confirmar una única llegada por entorno en la bandeja, registrar únicamente referencias y resultados —nunca secretos— y cerrar las casillas restantes de F12.
...
- `logic-reserva-preview` y `logic-reserva` existen. Preview sirve `6ac2fad2-0f49-4e7c-8e89-062fe586b081` y producción `238d0d16-f456-4831-a092-66c8f6f34f56`; ambos declaran `LEADS_RESEND_API_KEY` como secret cifrado y usan `marinerandreu+logic@gmail.com`. Tras la rotación, falta revalidar la entrega real de las claves nuevas.
